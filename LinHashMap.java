...
/************************************************************************************
 * The `LinHashMap` class provides hash maps that use the Linear Hashing
 * algorithm.
...
public class LinHashMap<K, V>
    ...
    private int h(Object key) {
        int hash = key.hashCode() % mod1;
        return (hash < 0) ? hash + mod1 : hash;
    }
    ...
    public V put(K key, V value) {
        ...
        if (i < 0 || i >= hTable.size()) {
            throw new IndexOutOfBoundsException("ERROR: Invalid bucket index " + i +
                    " for key " + key + ". Table size: " + hTable.size());
        }
